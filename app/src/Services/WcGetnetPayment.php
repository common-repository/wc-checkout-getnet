...
class WcGetnetPayment {
	public static function set_customer( WC_Order $order, $method = "credit" ) : stdClass {
		...
		$customer = [
            ...
            'billing_address'  => (object) self::get_checkout_address( $order, $method, 'billing')
        ];
	...
	public static function set_shipping( WC_Order $order, $method ): stdClass {
		return (object) [
			'address' => (object) self::get_checkout_address( $order, $method, 'shipping')
		];
	}

    public static function get_auth_token( $client_ID, $seller_ID, $client_secret, $data ): string {
		...
		if ( $order ) {
			$order->update_meta_data( '_getnet_transaction_auth_request', $request_data );
			$order->update_meta_data( '_getnet_transaction_auth_response', $response );
			$order->save();
		}
	...
	public static function get_checkout_address( $order, $method, $address_type = 'billing') {
	...
	public static function render_installments_options( $total, $installments, $initial_interest, $interest_increase, $no_interest ) {
		...
		for ( $times = 2; $times <= $installments; $times++ ) {
			...
			$selected = '';
			if( WC()->session->get( 'choiced_installments') && WC()->session->get( 'choiced_installments') == $times ) {
				$selected = 'selected';
			}

			$output .= sprintf( '<option value="%1$s" data-total=%2$s  %4$s data-fee=%5$s >%3$s</option>', $times, $value, $text, $selected, $interest );
		}
		return $output;
	}
