...
class WcGetnetApi {
	...
	public function send_request_post( $data, $access_token, $type, $method = 'POST' ) {
		$header          = $this->get_header( $data, $access_token );
	...
	public function create_meta_header_endpoint_order( $data, $header, $endpoint ) {
		$order_id = isset($data['order_id']) ? $data['order_id'] : $data['data']->order->order_id;
		...
		$order->update_meta_data( '_getnet_transaction_header', $header );
		$order->update_meta_data( '_getnet_transaction_endpoint', $endpoint );
		$order->save();
	}
	...
	public function send_request_post_v2( $data, $access_token, $type ) {
		$header          = $this->get_header( $data, $access_token );
		$environment_url = self::get_environment_url();
		$endpoint        = $environment_url . '/v2/payments' . $type;

		if ( ! $header ) {
			return new WP_Error('400',__( 'Erro ao processar o cabeçalho da requisição', 'wc_getnet' ));
		}
		...
		$this->create_meta_header_endpoint_order( $data, $header, $endpoint );
		return $apiResponse;
	}
	 ...
	 * @return array
	 */
	public function get_header( $data, $access_token = '' ) {
		if ( ! $access_token ) {
			$access_token = Services_Payment::get_auth_token( $this->client_id, $this->seller_id, $this->client_secret, $data );
		}
		...
		$this->handle_pix_headers($return);

		return $return;
	}

	/**
	 * Process handle pix headers - https://developers.getnet.com.br/idempotency#tag/Pix
	 *
	 * @param array  $headers Headers parameters.
	 * @return mixed
	 */
	public function handle_pix_headers( &$header ) {
		if( !WC()->session ) {
			return $header;
		}
		...
		$header['x-qrcode-expiration-time'] = $pix_settings["qrcode_expiration_time"];

		if ( isset($pix_settings['logs']) && $pix_settings['logs'] === 'yes' ) {
			Logs_Entity::get_pix_order( 'Header', $header );
		}
	...
	protected function process_billet_payment_step_2( $request ) {
		...
		return $order_controller->update_order_by_webhook( $request );
	}
	...
	public function get_webhook_args( $get ) {
		...
		if ( in_array( $response['status'], ['ERROR','DENIED' ] ) ) {
			$response['description_detail'] = isset( $get['description_detail'] ) ? wc_clean( $get['description_detail'] ) : '';
			$response['error_code']         = isset( $get['error_code'] ) ? wc_clean( $get['error_code'] ) : '';
		}
	...
	protected function is_valid_payment( $wc_order, $payment_id ) {
		$logs = $this->logs->logs_enabled( $wc_order );
