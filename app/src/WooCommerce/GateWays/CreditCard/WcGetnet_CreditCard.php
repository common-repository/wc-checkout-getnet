...
use WcGetnet\WooCommerce\GateWays\CreditCard\InstallmentsByValue;
...
class WcGetnet_CreditCard extends Abstract_Payment {
	...
	public $installmentsByValueConfig;
	...
	public function __construct() {
		...
		$this->installmentsByValueConfig = new InstallmentsByValue($this);
		...
		if ( is_admin() ) {
			...
			add_filter( 'woocommerce_settings_api_sanitized_fields_'. $this->id, [$this->installmentsByValueConfig, 'updateRequestData'], 10, 3 );
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields = array_merge(
			...
			$this->installmentsByValueConfig->prepareFieldsFromRequest($this->get_post_data())
		);
	...
	public function handleHeader() {
		?>
		<div class="gnt-header-container admin-config credit-card-admin-section">
			...
			<div class="gnt-header-item">
				...
				<div class="copy-link">
					...
					<div class="text-image">
						...
						<img class="img-copy copy-message-text" src="<?php echo esc_url( \WcGetnet::core()->assets()->getAssetUrl( 'images/copy.png' ) )?>">
					</div>
				</div>
			</div>
	...
	public function handleContent() {
		?>
			<div id="gntAdminCreditConfig" class="gnt-container"> 
				...
				<div class="gnt-group <?php echo $this->installmentsByValueConfig->isEnabled()?:'hidden'?>" id="gnt-installments-by-value-block">
					<p class="group-title">
						<b>Configuração de parcelamento por valor</b>
					</p>

					<div id="installments-by-value-group">
						<?php
							$this->installmentsByValueConfig->renderRepeaterFields();
						?>
					</div>
					<div class="clone-installments">
						<a href="javascript:void(0)" class="clone-installments-by-value">Adicionar novo</a>
					</div>
				</div>
			</div>
		<?php
	...
	public function process_payment( $order_id ) {
		...
		$card_brand               = Settings_Entity::post( $this->id . '-card_brand', FILTER_SANITIZE_STRING );
		...
		if( $this->isHomolog() && $card_brand = 'mastercard' && $number_installments >= 3 && $number_installments <= 6 ) {
			 $amount = $this->format_amout( $order, $number_installments ).'0'.$number_installments.'0'.$number_installments;
		}
		...
		$order->update_meta_data( '_getnet_payment_type', 'CREDIT' );
		...
		if( isset( $response['message'] ) && $response['message'] == 'Transacao negada.afpd!') {
			$status = 'DENIED';
			$details['description'] = 'Transação negada pelo anti-fraude, entre em contato com a Getnet';
		}

		if( isset( $response['reason_message'] ) ) {
			$details['description'] = $response['reason_message'];
		}
	...
	private function getCreditData( int $installments, array $cc ): stdClass {
		...
		if ( !empty($this->soft_descriptor) ) {
			$credCardData['soft_descriptor'] = iconv(mb_detect_encoding($this->soft_descriptor,'UTF-8, ASCII, ISO-8859-1'),'ASCII//TRANSLIT//INGORE',$this->soft_descriptor);
		}
	...
	protected function format_amout( $wc_order, $number_installments ) {
		...
		if ( $total_with_installments && $number_installments > 1) {
			return (int) round(( $total_with_installments * 100 ));
		}

		//bug fix for round down on cents ex: 1,15 was 114 in the last version.
		return (int) str_replace([',', '.'], '', $wc_order->get_total());
	}
