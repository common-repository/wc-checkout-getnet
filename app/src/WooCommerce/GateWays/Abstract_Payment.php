...
abstract class Abstract_Payment extends WC_Payment_Gateway {
	...
	public function isHomolog() {
		return $this->environment_name === 'homolog';
	}
	...
	protected function update_order_by_status( $wc_order, $status, $details = [] ) {
		...
		switch ( $status ) {
			...
			case 'NOT APPROVED':
				$order_controller->payment_failed( wc_clean( $description ) );
				break;

			default:
				$order_controller->payment_canceled( wc_clean( $description ) );
				break;
		}
	}
	...
	public function thank_you_page( $order_id ) {
		...
		if ( 'getnet-pix' === $this->id ) {
			...
			$args = [
				...
				'status_code' => $status,
				'creation_date_qrcode' => $resp_additional_data['creation_date_qrcode'],
				'expiration_date_qrcode' => $resp_additional_data['expiration_date_qrcode'],
			];
		}
	...
	protected function translate_status( $status , $description = '' ) {
		switch ( $status ) {
			case 'APPROVED':
				return __( 'Pagamento aprovado!', 'wc_getnet' );

			case 'NOT APPROVED':
				return __( 'Transação não autorizada!', 'wc_getnet' );

			case 'DENIED':
				$description = $description['description'] ?? __( 'Falha ao processar a transação, por favor consulte o suporte da loja!', 'wc_getnet' );
				return __( $description, 'wc_getnet' );

			case 'DENY':
				$description = $description['description'] ?? __( 'Transação não autorizada!', 'wc_getnet' );
				return __( $description, 'wc_getnet' );
			...
			default:
				return __( 'Falha no pagamento, por favor consulte o suporte da loja!', 'wc_getnet' );
		}
	}
