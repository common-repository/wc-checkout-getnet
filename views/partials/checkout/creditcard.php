...
use WcGetnet\WooCommerce\GateWays\WCGetnet_CreditCardFee;
...
<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-cc-form" class="wc-credit-card-form wc-payment-form" style="background:transparent;">
	...
	<?php if ( $this->is_min_value_from_installments() && $this->installments && 0 < intval( $this->installments ) ) : 
		$totalCheckout = new WCGetnet_CreditCardFee();
		$this->installments = $this->installmentsByValueConfig->checkInstallmentByValueRange($totalCheckout->get_total_cart_value());
	?>
		<div class="form-row form-row-wide">
			...
			<select id="<?php echo esc_attr( $this->id ) . '_number_installments'; ?>"
				...
				data-total="<?php echo esc_html( WC()->cart->total ); ?>"
				data-action="select2"
				data-required="true"
				data-element="installments">
				<?php echo WcGetnetPayment::render_installments_options( $totalCheckout->get_total_cart_value(), $this->installments, $this->installments_initial_interest, $this->installments_increase_interest, $this->installments_interest ); ?>
			</select>
		</div>
	<?php endif; ?>
